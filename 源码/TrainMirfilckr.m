function [W, U, V, B, obj] = TrainMirfilckr(X, Y, L, opt)
...
while i < maxItr
    ...
    %update B
    BA = W*W';
    BB = sparse(lambda1*D1+lambda2*D2);
    BC = W*L_s+lambda1*U'*X*S1'+lambda2*V'*Y*S2';
    B = lyap(BA,BB,-BC); 
    
    obj1 = norm((L_s-W'*B),'fro');
